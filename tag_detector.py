...
def tag(image):
    ...
    global mid_ad, com_angle, angle_of_msrr, error_of_angle, tag_id
...
# ------------------------------------------------ ↓ 設定4種情況下角度輸出 ↓ ------------------------------------------------ #
        ...
        # if bool_angle == True:
        #     cv2.putText(image, f'Error of angle : {str(round(error_of_angle, 2))}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        
        if len(angle_of_msrr) == 0:
            angle_of_msrr.append(com_angle)

        elif len(angle_of_msrr) == 1:
            angle_of_msrr.append(com_angle)
